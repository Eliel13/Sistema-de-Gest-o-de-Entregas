        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg">
                <div className="flex justify-between items-center p-5 border-b">
                    <h2 className="text-xl font-bold text-slate-800">{isEditing ? 'Editar Produto' : 'Adicionar Novo Produto'}</h2>
                    <button onClick={onClose} className="text-slate-400 hover:text-slate-800">{ICONS.x}</button>
                </div>
                <div className="p-6">
                    <p>Formulário para {isEditing ? `editar ${product?.name}` : 'novo produto'}.</p>
                </div>
